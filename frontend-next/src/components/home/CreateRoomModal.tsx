...
export function CreateRoomModal({ show, onClose, salanombre, setSalanombre, newRoomPin, setNewRoomPin, newRoomType, setNewRoomType, createRoom, creating }: Props) {
  if (!show) return null;
      ...
      <div className="w-full max-w-[420px] bg-slate-900/90 backdrop-blur-[28px] border border-white/20 rounded-2xl shadow-[0_24px_80px_rgba(0,0,0,0.7),inset_0_1px_0_rgba(255,255,255,0.14),0_0_0_1px_rgba(99,102,241,0.12)] p-8 flex flex-col gap-5 animate-fade-up">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="flex bg-black/40 border border-white/15 rounded-xl p-1 gap-1">
            <button className={`flex-1 py-2 rounded-lg text-sm transition-all ${newRoomType === 'texto' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-400/30' : 'text-slate-400 hover:bg-white/5'}`} onClick={() => setNewRoomType('texto')}>
              Texto
            </button>
            <button className={`flex-1 py-2 rounded-lg text-sm transition-all ${newRoomType === 'multimedia' ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-400/30' : 'text-slate-400 hover:bg-white/5'}`} onClick={() => setNewRoomType('multimedia')}>
              Multimedia
            </button>
          </div>
        </div>
