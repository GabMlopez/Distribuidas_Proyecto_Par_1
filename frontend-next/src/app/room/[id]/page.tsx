...
export default function RoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { userContext, updateToken } = useUser();

  const [roomName, setRoomName] = useState(roomId);
  const [roomType, setRoomType] = useState('texto');
  const [roomToken, setRoomToken] = useState('');
  const [isReady, setIsReady] = useState(false);
  const [loadingHistory, setLoadingHistory] = useState(true); // Nuevo estado
  ...
  const loadHistoricalMessages = useCallback(async () => {
    const finalRoomToken = roomToken || sessionStorage.getItem('room_token');
    if (!finalRoomToken) {
      console.log('No hay token, no se puede cargar historial');
  ...
  }, [roomId, roomToken, API]);

  // PRIMER useEffect: Restaurar datos
  useEffect(() => {
    ...
    console.log('Restaurando datos:', { savedNickname, savedRoomToken, savedRoomName, savedRoomType });

    if (savedRoomToken && !roomToken) {
      setRoomToken(savedRoomToken);
      if (userContext.token !== savedRoomToken) {
        updateToken(savedRoomToken);
      }
    }
  ...
  useEffect(() => {
    ...
    let finalRoomToken = roomToken;
    if (!finalRoomToken) {
      const savedToken = sessionStorage.getItem('room_token');
      if (savedToken) {
        finalRoomToken = savedToken;
    ...
    console.log('Conectando WebSocket con:', { nickname: finalNickname, roomId, deviceId: finalDeviceId });
    ...
    websocket.onopen = () => {
      ...
      setWsStatus('open');
      // Cargar historial DESPUÉS de conectar WebSocket
    ...
    websocket.onmessage = (e) => {
      try {
        ...
        if (data.type === 'user_list' || data.tipo === 'user_list') {
          ...
          setMessages(prev => {
            // Si es un mensaje nuevo que ya existe en el historial, no lo duplicamos
    ...
    const handleBeforeUnload = () => {
      if (websocket.readyState === WebSocket.OPEN) {
        const payload = JSON.stringify({ sala_id: roomId, nickname: finalNickname, device_id: finalDeviceId });
  ...
  }, [roomId, userContext?.nickname, userContext?.deviceId, roomToken, isReady, router, loadHistoricalMessages, API]);

  const sendMessage = () => {
  if (!currentMessage.trim() || !ws || ws.readyState !== WebSocket.OPEN) return;
  
  // Formato correcto de mensaje que espera el backend
  const message = { 
    tipo: 'chat',      // El backend espera 'tipo' no 'type'
  ...
  // Mostrar loading mientras se restauran los datos
